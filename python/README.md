...
Phụ thuộc duy nhất là `numpy` (khai báo trong `pyproject.toml`, khoá ở `uv.lock`).

## Chạy thử

Yêu cầu Python ≥ 3.8. Khuyến nghị dùng [uv](https://docs.astral.sh/uv/) — nó tự tạo môi trường ảo và cài `numpy` theo `uv.lock`:
...
Không dùng uv thì tự cài phụ thuộc rồi chạy bằng Python hệ thống:
