...
\begin{itemize}
    \item \texttt{src/}: package Julia \texttt{FrequentItemsetMining} (FP-Max và baseline naive-maximal, FP-Growth cơ bản và tối ưu, sinh luật kết hợp, I/O).
...
Bộ kiểm thử gồm: kiểm tra parser ba định dạng; FP-Growth và FP-Max trên CSDL toy ở Chương \ref{sec:example}; đối chiếu bản tối ưu với bản cơ bản trên năm CSDL (hai toy và ba benchmark: chess, mushrooms, retail); kiểm tra sinh luật kết hợp; và benchmark đối chiếu base/opt trên chess. Lưu ý thuật ngữ: \texttt{base}/\texttt{opt} trong dòng \texttt{[bench]} của bộ test là \emph{FP-Growth cơ bản so với FP-Growth tối ưu} (engine FP-Growth), khác nghĩa với \texttt{base} (baseline naive-maximal) và \texttt{opt} (FP-Max trực tiếp) dùng trong thực nghiệm Chương \ref{sec:evaluation}. Các tỉ lệ tốc độ/bộ nhớ in trong dòng \texttt{[bench]} là số đo on-the-fly, phụ thuộc máy và lần chạy (JIT warm-up, GC) nên có thể dao động, không nhất thiết trùng với số liệu cố định trong báo cáo.
